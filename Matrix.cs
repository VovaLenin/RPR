...
namespace RPR
{
    public class Matrix
    {
        ...
        /// <returns></returns>
        public static Matrix BubbleSort(Matrix mas, int masLength,int ind)
        {
            double temp, temp_ind;

            for (int i = 0; i < masLength; i++)
            {
                for (int j = i + 1; j < masLength; j++)
                {
                    if (mas[i, ind] > mas[j, ind])
                    {
                        temp = mas[i, 0];
                        temp_ind = mas[i, 1];
                        mas[i, 0] = mas[j, 0];
        ...
        public static Matrix ToSort(Matrix mas_temp, int masLength)
        {
            int k=0;
            Matrix mas = new Matrix(masLength,2);
       
            for (int i = 0; i < masLength; i=i+2)
            {

                if (mas_temp[i, 0] < mas_temp[i + 1, 0])
                {
                    mas[k, 0] = mas_temp[i, 0];
                    mas[k, 1] = mas_temp[i, 1];
                }
                else {
                    mas[k, 0] = mas_temp[i + 1, 0];
                    mas[k, 1] = mas_temp[i + 1, 1];
                }
                ++k;
        ...
        public static double SDMSrednee(Matrix mas_temp, int masLength)
        {
            double s = 0;
